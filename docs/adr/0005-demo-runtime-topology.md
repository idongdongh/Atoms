...
- 预览网关：API 按项目代理子进程 dev server；端口按项目确定性分配；预览 URL 稳定不变（避免 origin 漂移导致 localStorage 与登录态丢失）；访问携带短期签名 token；预览使用独立 Origin 与主站分域。文件变更默认依赖 HMR，重启仅由用户或 Agent 显式触发。参考 Dyad 已验证实现：`dyad/src/ipc/services/app_runtime_service.ts`、`dyad/shared/ports.ts`、`dyad/worker/proxy_server.js`。
