...
- 在线体验：<在线地址>（注册任意邮箱即可；演示账号 `<demo 账号>` / `<demo 密码>`）
- 代码仓库：<GitHub 仓库地址>
...
4. **延展能力：发布与回退**：点发布 → 受控 `vite build`（固定 `--base`）→ 不可变 release 目录 → 数据库激活指针原子切换 → 获得 `/published/<id>/` 公网 URL；任意历史 release 一键激活即秒级回退。
5. **稳定性（演示时可现场触发）**：预览 dev server 空闲 10 分钟自动回收（2C2G 内存友好），再次打开项目自动唤醒；预览启动失败自动重试一次。
...
| 生成应用数据接 Supabase                         | ✅                           | 计划中（M-B1，等凭据）：anon key + RLS 直连，service_role 不出服务端（[ADR 0006](docs/adr/0006-generated-app-data-and-publish.md)） |
...
| 生成应用数据后端（Supabase lite） | ⏸    | 代码就绪前的最后一块：等 Supabase 项目凭据（M-B1，预计一个工作日内）                                             |
| 在线环境                          | ⏸    | 服务器已选定（腾讯云香港 2C2G），等域名解析后 `docker compose up` 即上线                                         |

**质量**：55 个单元/集成测试全绿（`pnpm check` = 格式 + 类型 + 测试 + 构建），核心路径（API 隔离、发布回退、预览生命周期）均有测试覆盖；第一阶段结束后做过一轮对抗式审查并修复 8 项并发/安全问题。

## 4. 如果继续投入（按优先级）

1. **M-B1 全栈生成**：模板集成 supabase-js（anon + RLS），Agent 增加建表/迁移受控工具 → "一句话生成带数据库 CRUD 的应用"。
2. **M-部署验收**：上线 + 真模型冒烟 + 基线数据（DeepSeek 生成成功率、预览就绪延迟）。
